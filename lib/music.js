...
var Music = function() {
  var self = this;
  self.path = '/music/archive';
  self.songs = {};
  self.emitter = new EventEmitter();
  self.used = 0;
  fs.chownSync(dir + '/bandwidth', uid, gid);
...
*/
  ...
  server.get('/musicm/2/list', function(req, res) {
    ...
    */
    //log(util.inspect(temp, false, 2, true));
    res.end(JSON.stringify(temp));
  });
  ...
  server.get('/musicm/bandwidth', function(req, res) {
    res.end('' + self.used);
  });
  ...
  self.storeUsed = function(amount) {
    self.used += amount;
